...
float ring(vec2 p, float ri, float cut, float t0, float px) {
  ...
  float blurMul = 1.0 + uBlur * 0.4;
  float th = max(1.0 - a, 0.5) * px * uLineThickness * blurMul;
  float h = (1.0 - smoothstep(th, th * (1.5 + uBlur * 0.5), d)) + 1.0;
  d += pow(cut * a, 3.0) * r;
  return h * exp(-uAttenuation / blurMul * d) * fade(t);
}
...
void main() {
  ...
  // Trim the infinite mathematical tail left by exp() so the canvas reaches true 0% alpha 
  // well before the canvas boundary, destroying the grey/dark box artifact.
  float alpha = smoothstep(0.01, 0.04, maxChannel) * uOpacity;
