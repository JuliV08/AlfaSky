...
export default function Stepper({
  ...
  const handleComplete = () => {
    setDirection(1);
    updateStep(1); // Mantiene el ciclo reiniciándolo al paso 1 en lugar de cerrar todo
  };

  return (
    <div className="outer-container" {...rest}>
      <div className={`step-circle-container premium-border glass-card ${stepCircleContainerClassName}`} style={{ padding: '0 0' }}>
    ...
    <motion.div onClick={handleClick} className="step-indicator" animate={status} initial={false}>
      <motion.div
        variants={{
          ...
          active: { scale: 1, backgroundColor: '#2E7DD1', color: '#fff' },
          complete: { scale: 1, backgroundColor: '#2E7DD1', color: '#fff' }
        }}
...
function StepConnector({ isComplete }: any) {
  const lineVariants = {
    ...
    complete: { width: '100%', backgroundColor: '#2E7DD1' }
  };
